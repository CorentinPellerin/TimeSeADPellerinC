...
@experiment.config
def config():
    # --- Paramètres généraux de l'expérience d'évaluation ---
    params = dict(
        # Nom du script d'entraînement à appeler (doit contenir get_test_pipeline et get_batch_dim)
        training_experiment='train_malhotra_APL',  # ⬅️ Ton script d'entraînement

        # Métrique utilisée pour sélectionner le meilleur modèle (sur validation)
        validation_metric='best_f1_score',

        # Métriques à évaluer sur le set de test
        evaluation_metrics=['best_f1_score', 'auprc'],

        # Batch et device
        batch_size=128,
        device='cpu',  # change en 'cuda:0' si dispo
        ...
        # Cross-validation
        test_folds=5,  # peut être réduit à 3 pour aller plus vite
        padding=1,

        # ID d'entraînements précédents à réutiliser (None = réentraîne)
        train_ids=None
    )

    # --- Grille de paramètres à tester (grid search) ---
    training_param_grid = dict(
        model_params=dict(
            lstm_hidden_dims=[[30, 20], [50, 30]],
            linear_hidden_layers=[[], [40], [20, 20],[40, 40]]
        ),
        training=dict(
            optimizer=dict(
                args=dict(
    ...
    # --- Valeurs fixes à appliquer (surchargées si présent dans grid) ---
    training_param_updates = dict(
        training=dict(
            ...
            trainer_hooks=[('post_validation', 'timesead.optim.trainer.EarlyStoppingHook')]
        )
)

    # --- Paramètres du détecteur à tester (ex: Mahalanobis only) ---
    detector_param_grid = dict(
        detector_type=["malhotra"]  # ou autre selon ce que tu veux évaluer
    )
...
def main(params, training_param_grid, training_param_updates, detector_param_grid, dataset, _run, _seed):
    ...
    for val_fold in range(params.test_folds):
        ...
        for test_fold in test_fold_indices:
            ...
            for metric in params.evaluation_metrics:
                test_score, info = getattr(test_evaluator, metric)(labels, scores) # test_score, info = test_evaluator.__getattribute__(metric)(labels, scores)
